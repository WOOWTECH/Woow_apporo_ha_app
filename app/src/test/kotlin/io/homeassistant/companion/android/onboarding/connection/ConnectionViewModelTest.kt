...
class ConnectionViewModelTest {
    ...
    fun `Given a valid http url when buildAuthUrl then urlFlow emits correct auth url and isLoading is false`(baseUrl: String) = runTest {
        ...
        turbineScope {
            ...
            val expectedAuthUrl = "$baseUrl/auth/authorize?response_type=code&client_id=${AuthenticationService.CLIENT_ID}&redirect_uri=apporohome://auth-callback"
            advanceUntilIdle()
    ...
    fun `Given auth callback uri with code when shouldRedirect then emits Authenticated event with mTLS status and returns true`(requireMTLS: Boolean) = runTest {
        ...
        val stringUri = mockAuthCodeUri(scheme = "apporohome", host = "auth-callback", authCode = authCode)
    ...
    fun `Given auth callback uri without code when shouldRedirect then no event and returns false`() = runTest {
        val stringUri = mockAuthCodeUri(scheme = "apporohome", host = "auth-callback", authCode = null)
        ...
        turbineScope {
            ...
            val request = mockk<WebResourceRequest> {
                every { url } returns mockk<Uri> {
                    every { this@mockk.toString() } returns "http://homeassistant.local:8123/auth/authorize?response_type=code&client_id=${AuthenticationService.CLIENT_ID}&redirect_uri=apporohome://auth-callback"
                }
            }
    ...
    fun `Given received error when onReceivedError is invoked then errorFlow emits appropriate error`() = runTest {
        ...
        turbineScope {
            ...
            val request = mockk<WebResourceRequest> {
                every { url } returns mockk<Uri> {
                    every { this@mockk.toString() } returns "http://homeassistant.local:8123/auth/authorize?response_type=code&client_id=https://aiot.apporo.io/android&redirect_uri=apporohome://auth-callback"
                }
            }
